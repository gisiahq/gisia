...
class ProjectTeam
  ...
  def add_role(user, role, current_user: nil)
    public_send(:"add_#{role}", user, current_user: current_user)
  end
  ...
  def find_member(user_id)
    ...
    member = group.members.find_by(user_id: user_id) if group && !member
  ...
  def add_member(user, access_level, current_user: nil, expires_at: nil)
    Members::Projects::CreatorService.add_member(
      project.namespace,
      user,
      access_level,
      current_user: current_user,
  ...
  def max_member_access_for_user_ids(user_ids)
    Gitlab::SafeRequestLoader.execute(
      resource_key: project.namespace.max_member_access_for_resource_key(User),
  ...
  def write_member_access_for_user_id(user_id, project_access_level)
    project.namespace.merge_value_to_request_store(User, user_id, project_access_level)
  end

  def purge_member_access_cache_for_user_id(user_id)
    project.namespace.purge_resource_id_from_request_store(User, user_id)
  end

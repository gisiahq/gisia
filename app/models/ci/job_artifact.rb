...
module Ci
  class JobArtifact < ApplicationRecord
    ...
    after_commit :store_file_after_transaction!, on: %i[create update], if: :store_after_commit?
    ...
    scope :for_sha, ->(sha, project_id) {
      joins(job: :pipeline).merge(Ci::Pipeline.for_sha(sha).for_project(project_id))
    }
    ...
    def self.file_types_for_report(report_type)
      Enums::Ci::JobArtifact.report_file_types.fetch(report_type) do
        raise ArgumentError, "Unrecognized report type: #{report_type}"
      end
    end
